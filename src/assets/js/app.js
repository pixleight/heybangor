...
// retweets.
if( $('#heybangor').length > 0 ) {
  twitterFetcher.fetch('468552211291598848', 'heybangor', 20, true, true, true, dateFormatter, false, handleTweets, false);
}
...
function handleTweets(tweets){
    var x = tweets.length;
    var n = 0;
    var element = document.getElementById('heybangor');
    var html = '<ul class="grid-block small-block-grid-1 medium-block-grid-3 large-block-grid-4">';
    while(n < x) {
      html += '<li><article class="tweeeet">' + tweets[n] + '</article></li>';
      n++;
    }
    html += '</ul>';
    element.innerHTML = html;

    $('.tweeeet a').attr('target', '_blank');
}
